...
# Basic Attention mechanism (replaces topic-aware attention)
class BasicAttention(nn.Module):
    ...
    def forward(self, text_hidden):
        ...
        scores = self.linear(text_hidden).squeeze(-1)  # (batch_size, seq_len)
        attn_weights = torch.softmax(scores, dim=1)  # (batch_size, seq_len)
        context_vector = torch.bmm(attn_weights.unsqueeze(1), text_hidden).squeeze(1)  # (batch_size, hidden_size)
        return context_vector, attn_weights


# Updated StanceDetectionModel13 with Basic Attention
class StanceDetectionModel13(nn.Module):
    def __init__(self, vocab_size, embedding_dim, hidden_size, num_classes, max_seq_len=512, dropout=0.3):
        ...
        self.attention = BasicAttention(hidden_size)  # Use Basic Attention now
    ...
    def forward(self, text, topic):
        ...
        context_vector, attention_weights = self.attention(text_output)  # Basic attention on text output only
    ...
    with torch.no_grad():
        for batch in data_loader:
            ...
            for i in range(min(len(texts), num_examples - count)):
                ...
                attn = attention_weights[i, :len(text_words)].cpu().tolist()
...
def train_model(model, train_loader,  criterion, optimizer, device, num_epochs=10, patience=3):
    ...
    for epoch in range(num_epochs):
        ...
        # Because val_loader not provided in code, skipping validation phase here
        # Could be added if validation data available
...
# And then the evaluation, plotting functions remain unchanged from your original code.
# For brevity, I will include these without modification below.
...
def evaluate_and_visualize(model, test_loader, device, word_to_idx, class_names):
    ...
    try:
        ...
        idx_to_word = {idx: word for word, idx in word_to_idx.items()}
        analyze_attention_weights(model, test_loader, device, word_to_idx, class_names, idx_to_word, num_examples=5)
        return metrics
    except Exception as e:
...
def main():
    ...
    results = {
        "model": "Mini-GRU + Basic Attention (GloVe)",
    ...
    print("✅ Results saved: experiment_results/min_gru_basic_attention_glove.json")
