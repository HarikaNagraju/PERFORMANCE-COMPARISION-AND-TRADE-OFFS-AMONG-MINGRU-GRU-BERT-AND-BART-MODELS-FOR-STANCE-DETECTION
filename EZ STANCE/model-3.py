...
#     df["label"] = label_encoder.transform(df["Stance 1"])
#     df["text"] = df["Target 1"] + " [SEP] " + df["Text"]
...
# def save_bert_results_and_visuals(preds, labels, class_names, metrics_dict, output_dir="experiment_results", model_name="bert"):
...
#     label_encoder.fit(train_df["Stance 1"])
...
#     save_bert_results_and_visuals(preds, labels, class_names, metrics)

#     print("BERT stance detection completed and results saved!")
...
def main():
    ...
    metrics["training_time_sec"] = training_time
    metrics["inference_time_sec"] = inference_time
    metrics["trainable_parameters"] = total_params
