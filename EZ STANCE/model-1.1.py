...
class SelfAttentionOverGRU(nn.Module):
    ...
    def forward(self, text_hidden, topic_hidden):
        ...
        # Expand topic vector across the sequence
        topic_expanded = topic_hidden.unsqueeze(1).expand(-1, seq_len, -1)  # (B, T, H)

        # Combine text + topic representations
        combined = torch.cat((text_hidden, topic_expanded), dim=2)  # (B, T, 2H)

        # Apply attention MLP: linear → tanh → linear
        scores = self.linear2(self.tanh(self.linear1(combined)))  # (B, T, 1)
        attn_weights = torch.softmax(scores, dim=1)  # Normalize over seq

        # Compute weighted context vector
        context_vector = torch.sum(attn_weights * text_hidden, dim=1)  # (B, H)

        return context_vector, attn_weights

class AdvancedSelfAttention(nn.Module):
    def __init__(self, hidden_size):
        super(AdvancedSelfAttention, self).__init__()
    ...
    def forward(self, gru_outputs):
        ...
        attn_scores = self.linear2(self.tanh(self.linear1(gru_outputs)))  # (batch, seq_len, 1)
        attn_weights = torch.softmax(attn_scores, dim=1)  # (batch, seq_len, 1)
        context = torch.sum(attn_weights * gru_outputs, dim=1)  # (batch, hidden)
...
class StanceDetectionModel(nn.Module):
    def __init__(self, vocab_size, embedding_dim, hidden_size, num_classes, word_to_idx=None,
        ...
        self.attention = AdvancedSelfAttention(hidden_size)
    ...
    def forward(self, text, topic):
        ...
        context_vector, attention_weights = self.attention(text_output)
...
def main():
    ...
    # Load data
    train_df = pd.read_csv("claim_raw_train_all_onecol.csv")
    val_df = pd.read_csv("claim_raw_val_all_onecol.csv")
    test_df = pd.read_csv("claim_raw_test_all_onecol.csv")

    # Build vocab
    all_texts = list(train_df["Text"]) + list(train_df["Target 1"])
    word_to_idx = build_vocab(all_texts, min_freq=2)
    vocab_size = len(word_to_idx)

    # Load GloVe
    glove_path = r"C:/Users/CSE RGUKT/Desktop/Major_Project/glove.42B.300d/glove.42B.300d.txt"  # Adjust path if needed
    ...
    # Datasets
    train_dataset = StanceDataset(train_df["Text"].tolist(), train_df["Target 1"].tolist(),
                                   train_df["label_encoded"].tolist(), word_to_idx)
    val_dataset = StanceDataset(val_df["Text"].tolist(), val_df["Target 1"].tolist(),
                                 val_df["label_encoded"].tolist(), word_to_idx)
    test_dataset = StanceDataset(test_df["Text"].tolist(), test_df["Target 1"].tolist(),
    ...
    model = StanceDetectionModel(vocab_size, embedding_dim, hidden_size, num_classes,
    ...
    # Load best model
    best_model = StanceDetectionModel(vocab_size, embedding_dim, hidden_size, num_classes,
                                      word_to_idx=word_to_idx,
                                      pretrained_embeddings=embedding_tensor,
                                      freeze_embeddings=True).to(device)
    ...
    # Save results
    results = {
        "model": "Mini-GRU + Attention (GloVe)",
    ...
    with open("experiment_results/min_gru_basicattention_glove.json", "w") as f:
        json.dump(results, f, indent=4)
    print("✅ Full experiment results saved at: experiment_results/min_gru_basicattention_glove.json")
