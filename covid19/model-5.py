# # model-5.py : BART for Stance Detection with Metrics, ROC, and Timing (Fixed Version)
...
#     # ✅ Ensure logits are 2D: (samples, classes)
#     if isinstance(logits, (tuple, list)):
#         logits = logits[0]  # take only the logits

#     logits = np.asarray(logits)
#     if logits.ndim == 3:
#         logits = logits[:, 0, :]  # squeeze batch dim if needed
...
#     df_train = pd.read_csv("trainmerged.csv")
#     df_val = pd.read_csv("valmerged.csv")
#     df_test = pd.read_csv("testmerged.csv")
...
#         "f1_score": results.get("eval_f1", 0.0)
#     }
...
class StanceDatasetBART(Dataset):
    def __init__(self, df, tokenizer, label_encoder, max_len=128):
        self.encodings = tokenizer(
            (df['Target 1'] + " [SEP] " + df['Tweet']).tolist(),
            padding=True,
            truncation=True,
            max_length=max_len,
            return_tensors='pt'
...
def main():
    ...
    df_train = pd.read_csv("C19_train.csv")
    df_val = pd.read_csv("C19_val.csv")
    df_test = pd.read_csv("C19_test.csv")
    ...
    # Save JSON
    Path("experiment_results").mkdir(exist_ok=True)
    with open("experiment_results/bart(covid).json", "w") as f:
        json.dump(metrics_json, f, indent=4)
